...
const DEFAULT_KEYCHAIN_SERVICE = 'job-search-automations';
const LEGACY_KEYCHAIN_SERVICE = 'job-search-automation';
const V0_KEYCHAIN_SERVICE = 'job-search-plugin';
const DEFAULT_KEYCHAIN_ACCOUNT = 'neon-connection-string';
const DEFAULT_CONFIG_PATH = fs.existsSync(path.join(os.homedir(), '.config', 'job-search-automations', 'config.json'))
  ? path.join(os.homedir(), '.config', 'job-search-automations', 'config.json')
  : fs.existsSync(path.join(os.homedir(), '.config', 'job-search-automation', 'config.json'))
  ...
  try {
    ...
    if (service === DEFAULT_KEYCHAIN_SERVICE) {
      try {
        ...
        try {
          const stdoutV0 = execFileSync(
            'security',
            ['find-generic-password', '-s', V0_KEYCHAIN_SERVICE, '-a', account, '-w'],
            { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] }
          );
          return stdoutV0.trim() || null;
        } catch {}
      }
    }
    return null;
...
async function main() {
  ...
  mode = mode || 'local';
  workflowDataPath = workflowDataPath || (fs.existsSync(path.join(os.homedir(), '.local', 'share', 'job-search-automations')) ? path.join(os.homedir(), '.local', 'share', 'job-search-automations') : path.join(os.homedir(), '.local', 'share', 'job-search-automation'));
