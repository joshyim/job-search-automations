...
7. **Intermediate Crawl Data & Ephemeral Files (`.job-search/tmp/`)**:
   ...
   - Pre-granted permissions in `.claude/settings.json` explicitly cover crawler script execution (`node ... crawl-job-board.js:*`), `mkdir`, `rm`, and `tee` for `.job-search/tmp/*`, scoped read/write for the workspace and `.job-search/**`, all `mcp__job-search-db__*` MCP database tools, and web access fallback (`WebSearch`, `WebFetch`) to guarantee scheduled unattended runs without prompts.
