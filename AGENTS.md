...
5. **Scheduled & Unattended Execution Guidelines (Cost & Scoping Control)**:
   ...
   - **ATS Platform Pre-Tagging Architecture (PRO-66)**:
     - The `companies` and `crawl_queue` schemas include an `ats_platform` column (normalized lowercase: `ashby`, `greenhouse`, `lever`, `workday`, `smartrecruiters`, `rippling`, `bamboohr`, `jazzhr`, `icims`, `jobvite`, `breezy`, `workable`, `recruitee`, `custom`, or `null`).
     - Both SQLite and Neon automatically backfill `ats_platform` during migrations and auto-detect it on `add_company` / `add_to_queue` if omitted.
     - **Upfront Strategy Selection**: Agents inspect `ats_platform` upfront on both companies (during crawl) and pending queue items (during assess):
       - **`ashby`**: **NEVER use `WebFetch`**. Ashby pages are JavaScript-rendered SPA shells that return empty HTML, wasting requests and risking loop detector circuit breakers. Immediately use the unauthenticated public board API (`/posting-api/job-board/<boardSlug>`) or `crawl-job-board.js "<url>" [--posting] --json`. Never call `/posting-api/job/{id}` directly (requires customer auth and returns 401).
       - **`greenhouse`**: Direct fetch (server-rendered HTML) or Greenhouse public API (`https://boards-api.greenhouse.io/v1/boards/<boardToken>/jobs`) or `crawl-job-board.js`.
       - **`lever`**: Direct fetch or Lever public API (`https://api.lever.co/v0/postings/<boardToken>?mode=json`) or `crawl-job-board.js`.
       - **`workday` / `smartrecruiters` / other ATS**: Run `crawl-job-board.js "<url>" [--posting] --json`.
       - **`custom` / undetermined**: Run `crawl-job-board.js "<url>" [--posting] --json`. If exit code 3 (`js_required`), fast-fail immediately without retry. If 0 listings, at most 1 targeted search fallback.
   ...
   - **Search Fallback Query Pacing & Rate Limit Guardrails (PRO-64)**:
     - Search fallback (`web_search` / `WebSearch`) is ONLY invoked if the lightweight crawler fails or yields 0 postings on custom non-ATS boards.
