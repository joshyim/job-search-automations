...
export class LocalAdapter {
    ...
    async listCompanies(includeExcluded = false) {
        ...
        const companies = table.rows.map(row => ({
            name: row['Company'] || '',
    ...
    async addCompany(data) {
        ...
        let existingIndex = table.rows.findIndex(r => (r['Company'] || '').toLowerCase() === data.name.toLowerCase());
        ...
        if (headers.includes('ATS Platform') || data.ats_platform) {
            if (!headers.includes('ATS Platform'))
                headers.splice(2, 0, 'ATS Platform');
    ...
    async updateCompany(currentName, updates) {
        ...
        const table = MarkdownTableParser.parseTable(content);
        const existingIndex = table.rows.findIndex(r => (r['Company'] || '').toLowerCase() === currentName.toLowerCase());
        if (existingIndex < 0) {
            throw new Error(`Company "${currentName}" not found`);
        }
        const row = table.rows[existingIndex];
        if (updates.name !== undefined)
            row['Company'] = updates.name;
        if (updates.careers_url !== undefined)
            row['Careers URL'] = updates.careers_url;
        if (updates.is_excluded !== undefined)
            row['Excluded'] = updates.is_excluded ? 'Yes' : 'No';
        if (updates.notes !== undefined)
            row['Notes'] = updates.notes || '';
        if (updates.last_searched_at !== undefined)
            row['Last Searched'] = updates.last_searched_at || '';
        const updatedContent = MarkdownTableParser.updateTableInContent(content, headers, table.rows);
        this.writeFile('target-companies.md', updatedContent);
        return {
            name: row['Company'],
    ...
    async excludeCompany(name, reason) {
        ...
        const headers = ['Company', 'Careers URL', 'Excluded', 'Notes', 'Last Searched'];
        const table = MarkdownTableParser.parseTable(content);
        let existingIndex = table.rows.findIndex(r => (r['Company'] || '').toLowerCase() === name.toLowerCase());
        if (existingIndex < 0) {
    ...
    async getBatch(limit) {
        ...
        all.sort((a, b) => {
            if (!a.last_searched_at && !b.last_searched_at)
                return 0;
            if (!a.last_searched_at)
                return -1;
            if (!b.last_searched_at)
                return 1;
    ...
    async listTitlePatterns(type) {
        ...
        const patterns = table.rows.map(r => ({
            pattern: r['Pattern'] || '',
            type: ((r['Type'] || '').toLowerCase() === 'exclude' ? 'exclude' : 'include'),
            level: r['Level'] || null,
            notes: r['Notes'] || null,
    ...
    async addTitlePattern(data) {
        ...
        const table = MarkdownTableParser.parseTable(content, '## Title Patterns');
        const existingIdx = table.rows.findIndex(r => (r['Pattern'] || '').toLowerCase() === data.pattern.toLowerCase() &&
    ...
    async updateTitlePattern(pattern, type, updates) {
        ...
        const headers = ['Pattern', 'Type', 'Level', 'Notes'];
        const table = MarkdownTableParser.parseTable(content, '## Title Patterns');
        const existingIdx = table.rows.findIndex(r => (r['Pattern'] || '').toLowerCase() === pattern.toLowerCase() &&
        ...
        if (updates.pattern !== undefined)
            row['Pattern'] = updates.pattern;
        if (updates.type !== undefined)
            row['Type'] = updates.type;
        if (updates.level !== undefined)
            row['Level'] = updates.level || '';
        if (updates.notes !== undefined)
            row['Notes'] = updates.notes || '';
        const updatedContent = MarkdownTableParser.updateTableInContent(content, headers, table.rows, '## Title Patterns');
        this.writeFile('target-job-titles-and-skills.md', updatedContent);
    ...
    async updateSkill(name, updates) {
        ...
        const headers = ['Skill', 'Category', 'Importance', 'Notes'];
        const table = MarkdownTableParser.parseTable(content, '## Skills');
        const existingIdx = table.rows.findIndex(r => (r['Skill'] || '').toLowerCase() === name.toLowerCase());
        if (existingIdx < 0) {
            throw new Error(`Skill "${name}" not found`);
        }
        const row = table.rows[existingIdx];
        if (updates.name !== undefined)
            row['Skill'] = updates.name;
    ...
    async updateRubricDimension(dimension, updates) {
        ...
        const idx = table.rows.findIndex(r => (r['Dimension'] || '').trim().toLowerCase() === target);
        if (idx < 0) {
    ...
    async removeRubricDimension(dimension) {
        ...
        const target = dimension.trim().toLowerCase();
        const initialLen = table.rows.length;
        table.rows = table.rows.filter(r => (r['Dimension'] || '').trim().toLowerCase() !== target);
        if (table.rows.length !== initialLen) {
            const updatedContent = MarkdownTableParser.updateTableInContent(content, headers, table.rows);
            this.writeFile('scoring-rubric.md', updatedContent);
            return true;
    ...
    async getCandidates(filters) {
        const content = this.readFile('job-candidates.md');
        const table = MarkdownTableParser.parseTable(content);
        let candidates = table.rows.map(r => {
            let parsedBreakdown = null;
            if (r['Breakdown']) {
                try {
        ...
        if (filters?.status) {
            candidates = candidates.filter(c => {
                if (c.status === filters.status)
                    return true;
                if (filters.status === 'in_progress' && (c.status === 'interviewing' || c.status === 'in progress'))
                    return true;
                if ((filters.status === 'not_pursuing' || filters.status === 'closed') && (c.status === 'not_pursuing' || c.status === 'closed' || c.status === 'rejected' || c.status === 'offer'))
                    return true;
                return false;
            });
        }
