...
export const DEFAULT_KEYCHAIN_SERVICE = 'job-search-automations';
export const LEGACY_KEYCHAIN_SERVICE = 'job-search-automation';
export const V0_KEYCHAIN_SERVICE = 'job-search-plugin';
 ...
 */
export function getKeychainSecret(service, account) {
    try {
 ...
 */
export function getNeonConnectionString(service = DEFAULT_KEYCHAIN_SERVICE, account = DEFAULT_KEYCHAIN_ACCOUNT) {
    ...
    if (!keychainSecret && service === DEFAULT_KEYCHAIN_SERVICE) {
        keychainSecret = getKeychainSecret(LEGACY_KEYCHAIN_SERVICE, account) ||
            getKeychainSecret(V0_KEYCHAIN_SERVICE, account);
    }
    if (keychainSecret) {
    ...
    throw new Error(`Neon database connection string could not be retrieved from macOS Keychain (service: "${service}", account: "${account}") or environment variable DATABASE_URL. ` +
