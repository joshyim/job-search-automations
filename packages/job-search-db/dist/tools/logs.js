import { z } from 'zod';
export function registerLogTools(server, workspaceManager) {
    server.tool('log_run', 'Record a pipeline search run with processed companies, queued URLs, and scored candidates', {
    ...
    server.tool('get_recent_runs', 'Retrieve recent pipeline run history', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
