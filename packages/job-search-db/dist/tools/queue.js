...
export function registerQueueTools(server, workspaceManager) {
    server.tool('check_url_exists', 'Check whether a job posting URL has already been queued or assessed', {
    ...
    server.tool('add_to_queue', 'Add a discovered job posting URL to the crawl queue with pending status', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    server.tool('get_pending_queue', 'Retrieve pending job URLs from the crawl queue, optionally filtered by company', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    server.tool('list_queue', 'Retrieve crawl queue entries, optionally filtered by status (pending, assessed, skipped) or company', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    server.tool('update_queue_status', 'Update the processing status of a URL in the crawl queue (e.g. assessed, skipped)', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
