import { z } from 'zod';
export function registerTitleTools(server, workspaceManager) {
    server.tool('list_title_patterns', 'List job title include and exclude patterns with target levels and notes', {
    ...
    }, async ({ selected_directory, directory, type }) => {
        try {
    ...
    server.tool('add_title_pattern', 'Add a new job title match pattern (include or exclude)', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    }, async ({ selected_directory, directory, pattern, type, level, notes }) => {
        try {
            const adapter = await workspaceManager.getAdapter(selected_directory || directory);
    ...
    server.tool('update_title_pattern', 'Update an existing job title match pattern, level, or notes', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    }, async ({ selected_directory, directory, current_pattern, current_type, pattern, type, level, notes }) => {
        try {
            const adapter = await workspaceManager.getAdapter(selected_directory || directory);
