import { z } from 'zod';
export function registerRubricTools(server, workspaceManager) {
    server.tool('get_scoring_rubric', 'Get the candidate evaluation scoring rubric with dimensions, weights, and rating criteria', {
    ...
    server.tool('update_rubric_dimension', 'Update an existing rubric dimension weight or tier descriptions (case-insensitive dimension matching)', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    server.tool('add_rubric_dimension', 'Add or upsert a scoring dimension to the candidate evaluation rubric (case-insensitive matching)', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
    ...
    server.tool('remove_rubric_dimension', 'Remove a dimension from the candidate evaluation rubric (case-insensitive matching)', {
        selected_directory: z.string().optional().describe('Path to the selected project workspace directory containing .job-search/'),
        directory: z.string().optional().describe('Alias for selected_directory'),
