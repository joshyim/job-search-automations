...
export function registerCandidateTools(server: McpServer, adapter: DataAdapter): void {
  server.tool(
    ...
    'Add or update a scored candidate job opportunity',
    {
      ...
      status: z.enum(['new', 'applied', 'interviewing', 'rejected', 'offer']).optional().describe('Pipeline status (default "new")'),
  ...
  server.tool(
    ...
    'Update the status of a candidate job posting (new, applied, interviewing, rejected, offer)',
    {
      url: z.string().describe('The URL of the candidate to update'),
      status: z.enum(['new', 'applied', 'interviewing', 'rejected', 'offer']).describe('New application status'),
  ...
  server.tool(
    ...
    {
      status: z.enum(['new', 'applied', 'interviewing', 'rejected', 'offer']).optional().describe('Filter by status'),
