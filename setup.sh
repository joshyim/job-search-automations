...
KEYCHAIN_SERVICE="job-search-automations"
...
# Auto-detect harness if not explicitly set
if [ -z "$HARNESS" ]; then
  is_codex_env=false
  if [ -n "$CODEX" ] || [ -n "$CODEX_SANDBOX" ] || [ -n "$CODEX_THREAD_ID" ] || [ -n "$CODEX_SESSION_ID" ] || [ -n "$CODEX_TASK_ID" ] || [ -n "$CHATGPT_TASK" ]; then
    is_codex_env=true
  fi
  is_claude_env=false
...
if [ -n "$INSTALL_TO" ] || [ -n "$DIRECTORY" ]; then
  ...
  node -e "
    ...
    for (const f of ['plugin.json', 'mcp.json', '.mcp.json', 'schema.sql', 'CLAUDE.md', 'AGENTS.md', 'installation-steps.md']) {
  ...
  if [ "$IS_CODEX_TARGET" = true ]; then
    ...
    node -e "
      ...
      const projectDir = '$PROJECT_DIR';
      const pluginDir = '$TARGET_PLUGIN_DIR';
      const tomlFile = '$CODEX_CONFIG_FILE';
      const startJs = path.join(pluginDir, 'packages', 'job-search-db', 'scripts', 'start.js');
      let rel = path.relative(projectDir, startJs);
      if (!rel.startsWith('./') && !rel.startsWith('../') && !rel.startsWith('/')) rel = './' + rel;
      ...
      if (fs.existsSync(tomlFile)) {
        try { content = fs.readFileSync(tomlFile, 'utf-8'); } catch {}
      }
      const block = [
    ...
    "
    echo -e "${GREEN}[+] Codex MCP configuration updated (least-privilege writes mode):${RESET} $CODEX_CONFIG_FILE"
    ...
    node -e "
      ...
      function copyRec(src, d) {
        ...
        if (stat.isDirectory()) {
          ...
          for (const entry of fs.readdirSync(src)) {
            copyRec(path.join(src, entry), path.join(d, entry));
          }
        } else {
    ...
    echo -e "${GREEN}[+] Native Codex skills installed:${RESET} $AGENTS_SKILLS_DIR"
...
echo -e "  Harness:           ${BOLD}$HARNESS${RESET}"
...
if [ "$IS_CODEX_TARGET" = true ] && [ -n "$CODEX_CONFIG_FILE" ]; then
  echo -e "  Codex Config:      ${BOLD}$CODEX_CONFIG_FILE${RESET}"
  echo -e "                     ${GREEN}✓${RESET} MCP Server:      ${CYAN}[mcp_servers.job-search-db]${RESET}"
  echo -e "                     ${GREEN}✓${RESET} Approval Policy: ${CYAN}default_tools_approval_mode = \"writes\"${RESET}"
  echo -e "  Codex Skills:      ${BOLD}$AGENTS_SKILLS_DIR${RESET}"
fi
if [ "$IS_CLAUDE_TARGET" = true ]; then
...
echo ""
echo -e "  ${BOLD}Model Class Recommendation:${RESET}"
if [ "$IS_CLAUDE_TARGET" = true ]; then
