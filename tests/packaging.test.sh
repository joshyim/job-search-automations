...
# conformance for job-search-automations.
# ==============================================================================
...
if (manifest.name !== 'job-search-automations') {
  console.error('Expected name to be job-search-automations, got: ' + manifest.name);
  process.exit(1);
}
...
pass "plugin.json contains required name: 'job-search-automations'"
...
assert_dir_exists "$PLUGIN_ROOT/packages/job-search-ui" "packages/job-search-ui/ directory exists at plugin root"
assert_file_exists "$PLUGIN_ROOT/packages/job-search-ui/public/index.html" "packages/job-search-ui/ contains SPA source (public/index.html)"
assert_file_exists "$PLUGIN_ROOT/packages/job-search-ui/public/styles.css" "packages/job-search-ui/ contains CSS styles (public/styles.css)"
assert_file_exists "$PLUGIN_ROOT/packages/job-search-ui/public/app.js" "packages/job-search-ui/ contains client application (public/app.js)"
assert_file_exists "$PLUGIN_ROOT/packages/job-search-ui/package.json" "packages/job-search-ui/ contains local server package.json"
assert_file_exists "$PLUGIN_ROOT/packages/job-search-ui/scripts/start.js" "packages/job-search-ui/ contains scripts/start.js launcher"
if [ -x "$PLUGIN_ROOT/packages/job-search-ui/scripts/start.js" ]; then
  pass "packages/job-search-ui/scripts/start.js is marked executable"
else
  fail "packages/job-search-ui/scripts/start.js is not executable"
fi

node -e "
const fs = require('fs');
const pkg = JSON.parse(fs.readFileSync('$PLUGIN_ROOT/packages/job-search-ui/package.json', 'utf-8'));
...
"
pass "packages/job-search-ui/package.json start script routes to node scripts/start.js"
...
if grep -q "## Runtime Architecture" "$PLUGIN_ROOT/CLAUDE.md" && \
   ...
   grep -q "\.claude/plugins/job-search-automations/" "$PLUGIN_ROOT/CLAUDE.md" && \
...
if (pl.name !== 'job-search-automations' || !pl.version) {
...
START_JS="$PLUGIN_ROOT/packages/job-search-ui/scripts/start.js"
