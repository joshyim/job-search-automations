...
assert_file_exists "$PROJECT_DIR/.claude/plugins/job-search-automations/CLAUDE.md" "CLAUDE.md installed in plugin directory (PRO-31)"
assert_file_exists "$PROJECT_DIR/.claude/plugins/job-search-automations/installation-steps.md" "installation-steps.md installed in plugin directory"
...
# ------------------------------------------------------------------------------
# Test 10: setup.sh --codex Integration (PRO-49)
# ------------------------------------------------------------------------------
echo -e "\n${BOLD}[Test 10] setup.sh --codex Integration (PRO-49)${RESET}"
CODEX_SH_DIR="$TMP_TEST_DIR/codex-sh-project"
mkdir -p "$CODEX_SH_DIR"

SETUP_CODEX_SH_OUT="$("$ROOT_DIR/setup.sh" --directory "$CODEX_SH_DIR" --resume "$FIXTURE_RESUME" --codex -y)"
assert_contains "$SETUP_CODEX_SH_OUT" "Setup Complete!" "setup.sh --codex reported completion"
assert_contains "$SETUP_CODEX_SH_OUT" "Codex Config:" "setup.sh displays Codex config in summary"
assert_contains "$SETUP_CODEX_SH_OUT" ".codex/config.toml" "setup.sh summary reports .codex/config.toml"
assert_contains "$SETUP_CODEX_SH_OUT" "Luna-class" "setup.sh summary recommends Luna-class models"
assert_contains "$SETUP_CODEX_SH_OUT" "/mcp" "setup.sh summary directs inspecting /mcp"

# Ensure Claude mentions not in Codex next steps
if [[ "$SETUP_CODEX_SH_OUT" == *"Claude Desktop"* ]] || [[ "$SETUP_CODEX_SH_OUT" == *"Claude Code"* ]]; then
  echo -e "  ${RED}✗${RESET} setup.sh --codex output contains Claude mentions in next steps"
  TEST_FAILED=$((TEST_FAILED + 1))
else
  echo -e "  ${GREEN}✓${RESET} setup.sh --codex output is free from Claude-only next steps"
  TEST_PASSED=$((TEST_PASSED + 1))
fi

assert_dir_exists "$CODEX_SH_DIR/.codex/plugins/job-search-automations" "Plugin installed in .codex/plugins/job-search-automations"
assert_file_exists "$CODEX_SH_DIR/.codex/config.toml" ".codex/config.toml created"

# Verify .codex/config.toml contents
CODEX_SH_TOML="$(cat "$CODEX_SH_DIR/.codex/config.toml")"
assert_contains "$CODEX_SH_TOML" "[mcp_servers.job-search-db]" ".codex/config.toml has job-search-db section"
assert_contains "$CODEX_SH_TOML" 'default_tools_approval_mode = "writes"' ".codex/config.toml sets default_tools_approval_mode = writes"

# Verify .agents/skills and AGENTS.md
assert_dir_exists "$CODEX_SH_DIR/.agents/skills" ".agents/skills/ directory created"
assert_file_exists "$CODEX_SH_DIR/.agents/skills/job-search-crawl/scripts/crawl-job-board.js" "Crawler script copied to job-search-crawl skill"
assert_file_exists "$CODEX_SH_DIR/AGENTS.md" "AGENTS.md created at root"

# Verify Claude-specific files are absent
if [ ! -d "$CODEX_SH_DIR/.claude" ]; then
  echo -e "  ${GREEN}✓${RESET} .claude/ directory omitted in setup.sh --codex"
  TEST_PASSED=$((TEST_PASSED + 1))
else
  echo -e "  ${RED}✗${RESET} .claude/ directory unexpectedly exists in setup.sh --codex"
  TEST_FAILED=$((TEST_FAILED + 1))
fi

if [ ! -f "$CODEX_SH_DIR/.mcp.json" ]; then
  echo -e "  ${GREEN}✓${RESET} .mcp.json omitted in setup.sh --codex"
  TEST_PASSED=$((TEST_PASSED + 1))
else
